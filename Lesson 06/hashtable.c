...
//La collisione avviene se con chiavi diverse ottengo lo stesso output
//In caso di collisione si va a creare un lista per ogni nodo.
//data.txt contiene matricola come chiave e poi i voti
...
//si passa come puntatore perchè dobbiamo aumentare il numero di chiavi nella
//tabella
...
//Restituisce il valore se presente
SearchResult search(HashTable hashTable, const char* key, int keyLen)
{
	...
	while(L)
	{
		if (keyLen == L->keyLen && memcmp(key, L->key, keyLen) == 0)
		{
			searchRes.i = i;
